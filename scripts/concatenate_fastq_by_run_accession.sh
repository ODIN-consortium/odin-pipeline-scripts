...
usage() {
    echo "Usage: $0 [OPTIONS] [minknow_data_dir] [output_dir] [run_accession] [metadata_file] [tmp_dir] [venv_folder]"
    echo ""
    echo "Concatenate FASTQ files for a specific run_accession."
    echo ""
    echo "Options:"
    ...
    echo "  1. Identify all barcodes for the specified run_accession"
    echo "  2. Concatenate fastq.gz files by barcode for the run_accession"
    echo "  3. Output one file per barcode in tmp_dir as: barcode_{run_accession}.fastq.gz"
    echo ""
...
set_config_value "${CONFIG_FILE}" run_accession "${run_accession}"
...
echo -e "\n\033[0;36m==========================================\033[0m"
echo -e "\033[0;36mConcatenating FASTQ files for run_accession: ${run_accession}\033[0m"
...
py_cmd="${PYTHON_CMD} \"${PY_SCRIPT}\" --minknow_data_dir \"${minknow_data_dir}\" --output_dir \"${output_dir}\" --run_accession \"${run_accession}\" --metadata_file \"${metadata_file}\" --tmp_dir \"${tmp_dir}\""
